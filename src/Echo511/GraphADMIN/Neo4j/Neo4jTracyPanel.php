...
class Neo4jTracyPanel implements \Tracy\IBarPanel
{
	...
	public function __construct(\GraphAware\Neo4j\Client\ClientBuilder $clientBuilder)
	{
		$clientBuilder->registerEventListener(
			\GraphAware\Neo4j\Client\Neo4jClientEvents::NEO4J_PRE_RUN,
			[$this, 'onPreRun']
		);
	}


	public function onPreRun(\GraphAware\Neo4j\Client\Event\PreRunEvent $event)
	{
		$this->statements = \array_merge($this->statements, $event->getStatements());
	}
...
X;
		...
		foreach ($this->statements as $query) {
			$code .= '<tr>';
			$code .= '<td>' . $query->text() . '</td>';
			$code .= '<td>' . \Tracy\Dumper::toHtml($query->parameters()) . '</td>';
			$code .= '</tr>';
		}
